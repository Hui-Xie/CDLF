...
NonconvexNet::NonconvexNet(const string& name, const string& saveDir, const vector<int>& layerWidthVector): FeedForwardNet(saveDir){
...
void NonconvexNet::build(){
    ...
    for(int i =1; i< nLayers; ++i){
        ...
        if (i != nLayers -1){
            ...
            addLayer(reLU);
            NormalizationLayer* normalLayer = new NormalizationLayer(layerID++, "NormLayer"+to_string(i),getFinalLayer(), getFinalLayer()->m_tensorSize);
