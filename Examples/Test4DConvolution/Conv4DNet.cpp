...
void Conv4DNet::build(){
    ...
    addLayer(conv1);
    ReLU* reLU1 = new ReLU(id++, "ReLU1", getFinalLayer(), {60,5, 5,5});
    addLayer(reLU1);
    NormalizationLayer* norm1 = new NormalizationLayer(id++, "Norm1",getFinalLayer(), {60,5, 5,5});
    addLayer(norm1);


    ConvolutionLayer* conv2 = new ConvolutionLayer(id++, "Conv2",getFinalLayer(), {60,5,3,3}, 1); //output 3*3
    addLayer(conv2);
    ReLU* reLU2 = new ReLU(id++, "ReLU2", getFinalLayer(), {3,3});
    addLayer(reLU2);
    NormalizationLayer* norm2 = new NormalizationLayer(id++, "Norm2",getFinalLayer(), {3,3});
    addLayer(norm2);
